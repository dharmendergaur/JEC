...
# Access the TTree named 'Events'
tree = file["Events"]


tree = R.TFile("Nano.root").Get("Events") 

# Define classes for dynamic branch handling
class Evt_br:
...
# # Create an instance of the Muon_br class with the collected branches
...
class Emu_br:
    def __init__(self, branches):
        for full_name, array in branches.items():
            attr_name = full_name.split("Muon_")[1]  # Get the part after "Muon_"
            setattr(self, attr_name, array)
...
#                 self.nTower = int(array[0])  # Assign scalar for nMuon
#             else:
...
#                 self.nCluster = int(array[0])  # Assign scalar for nMuon
#             else:
...
def run():
    ...
    parser.add_argument('--l1ntuple',              type=str, dest='l1ntuplePath', required=False, default="L1Ntuple.root", help="L1T ntuples")     
    ...
    parseGroup1.add_argument('--l1MatchGen', action='store_true', default=True)
    ...
    parseGroup2.add_argument('--l1NtuplePhiRing', action='store_true', default=True)
    parseGroup3 = parser.add_mutually_exclusive_group()
    parseGroup3.add_argument('--offlineCHSJet', action='store_true')
    parseGroup3.add_argument('--offlinePUPPIJet', action='store_true')
    parseGroup3.add_argument('--usingL1Nano', action='store_true', default=True)
        ...
        for jEvt in range(Ntot):
            ...
            for iJ in range(nOffJets):
                if offlinePUPPIJet:
                    ...
                    hOfflineJet_Pt_0.Fill(Jet_br.pt[iJ])
            ...
            if not isMC and len(HLT_Triggers_Required) > 0:
                ...
                for iMu in range(nOffMuons):
                    ...
                    if not Muon_br.tightId[iMu]: continue
            ...
            for iOff in range(nRefJets):
                ...
                if   l1MatchOffline:
                    ...
                    if dataEra in ['2022B', '2022C', '2022D', '2022E']:
                        ...
                        if offlinePUPPIJet:    
                            ...
                            reject_if = [
                                isCentralJet          and Jet_br.neHEF[iOff]  >= 0.90, # neutralHadronEnergyFraction()
                                isCentralJet          and Jet_br.neEmEF[iOff] >= 0.90, # jet_data->nemef.push_back(it->neutralEmEnergyFraction());
                                isCentralJet          and (Jet_br.chMultiplicity[iOff] + Jet_br.neMultiplicity[iOff]) <= 1, # jet_data->cMult.push_back(it->chargedMultiplicity()); jet_data->nMult.push_back(it->neutralMultiplicity());
                                isCentralJet          and Jet_br.muEF[iOff]   >= 0.80, # jet_data->mef.push_back(it->muonEnergyFraction());
                                isCentralJet          and Jet_br.chHEF[iOff]  <= 0.01, # jet_data->chef.push_back(it->chargedHadronEnergyFraction());
                                isCentralJet          and Jet_br.chMultiplicity[iOff] == 0, # jet_data->cMult.push_back(it->chargedMultiplicity());
                                isCentralJet          and Jet_br.chEmEF[iOff] >= 0.80, # jet_data->cemef.push_back(it->chargedEmEnergyFraction());
                                
                                isForwardCentralJet_1 and Jet_br.neHEF[iOff]  >= 0.90, # neutralHadronEnergyFraction()
                                isForwardCentralJet_1 and Jet_br.neEmEF[iOff] >= 0.99, # jet_data->nemef.push_back(it->neutralEmEnergyFraction());
                                isForwardCentralJet_1 and Jet_br.muEF[iOff]   >= 0.80, # jet_data->mef.push_back(it->muonEnergyFraction());
                                isForwardCentralJet_1 and Jet_br.chMultiplicity[iOff] == 0, # jet_data->cMult.push_back(it->chargedMultiplicity());
                                isForwardCentralJet_1 and Jet_br.chEmEF[iOff] >= 0.80, # jet_data->cemef.push_back(it->chargedEmEnergyFraction());
                                
                                isForwardCentralJet_2 and Jet_br.neEmEF[iOff] >= 0.99, # jet_data->nemef.push_back(it->neutralEmEnergyFraction());                             
                                isForwardCentralJet_2 and Jet_br.neMultiplicity[iOff] <= 1 , # jet_data->nMult.push_back(it->neutralMultiplicity());

                                isForwardJet          and Jet_br.neHEF[iOff]  <= 0.20, # neutralHadronEnergyFraction()
                                isForwardJet          and Jet_br.neEmEF[iOff] >= 0.90, # jet_data->nemef.push_back(it->neutralEmEnergyFraction());
                    ...
                    if dataEra in [
                        ...
                        if offlinePUPPIJet:    
                            ...
                            reject_if = [
                                isCentralJet          and Jet_br.neHEF[iOff]  >= 0.99, # neutralHadronEnergyFraction()
                                isCentralJet          and Jet_br.neEmEF[iOff] >= 0.90, # jet_data->nemef.push_back(it->neutralEmEnergyFraction());
                                isCentralJet          and (Jet_br.chMultiplicity[iOff] + Jet_br.neMultiplicity[iOff]) <= 1, # jet_data->cMult.push_back(it->chargedMultiplicity()); jet_data->nMult.push_back(it->neutralMultiplicity());
                                isCentralJet          and Jet_br.muEF[iOff]   >= 0.80, # jet_data->mef.push_back(it->muonEnergyFraction());
                                isCentralJet          and Jet_br.chHEF[iOff]  <= 0.01, # jet_data->chef.push_back(it->chargedHadronEnergyFraction());
                    ...
                    for iMu in range(nOffMuons):
                        ...
                        if not Muon_br.mediumId[iMu]: continue
                ...
                if l1MatchGen:
                    ...
                    for iOff in range(nOffJets):
                        ...
                        if offlinePUPPIJet:
                            ...
                            iOff_vec.SetPtEtaPhiM(Jet_br.pt[iOff], Jet_br.eta[iOff], Jet_br.phi[iOff], 0)
                        else:
                ...
                for src in ['emu']: # ['unp','emu']: not necessary now
                    l1TP_br = None
                    if src == 'unp':
                        l1TP_br = uTP_br
                    elif src == 'emu':
                ...
                for src in ['emu']:
                    ...
                    print(" Hi ")
                    print("    %s %s nTT %g" % (" " * 4, src, l1TT_br.nTower)    )
